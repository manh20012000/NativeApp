...
const PesionChat = ({ route, navigation }) => {
  ...
  const onSend = useCallback(async (messages = []) => {
    try {
      ...
      console.error("Error sending message to server11:", error);
    }
  }, []);
  //change buutobsen
  const handlerInfor_user = () => {
    ...
    <View style={{ flex: 1, backgroundColor: "black" }}>
      ...
      {chooseLibrary ? (
        <View style={{ flex: 1, backgroundColor: "black" }}>
          ...
          > */}
          <View style={{ backgroundColor: 'pink', flex: 0.95 }}>
            <GiftedChat
              messages={messages}
              onSend={(newMessages) =>
